...
class errorneous
{
  ...
  var token0 : map<errorneous, nat>, token1 : map<errorneous, nat>;

  method swap(amount1Out : nat, to : errorneous)
    requires this in token0 && this in token1
    requires token1[this] == reserve1
    requires reserve0 - amount0Out == token0[this] && reserve0 > amount0Out >= 0
    requires amount0Out > 0 && reserve1 > 0 && reserve0 > 0

    modifies this

    ensures reserve0 * reserve1 >= old(reserve0) * old(reserve1)
  {
    if(0 < amount1Out < token1[this])
    {
